...
def create_user(
    *,
    ...
    if existing:
        raise HTTPException(status_code=400, detail="Un utilisateur avec ce nom ou email existe déjà")
    ...
    try:
        ...
        logger.warning("Could not send welcome email to %s: %s", db_user.email, e)
...
def approve_user(
    *,
    ...
    try:
        ...
        logger.warning("Could not send approval email to %s: %s", user.email, e)
...
def delete_user(
    *,
    ...
    try:
        ...
        raise HTTPException(
            status_code=400,
            detail="Impossible de supprimer cet utilisateur car il est lié à des commandes. Désactivez-le à la place.",
        )
    return user
